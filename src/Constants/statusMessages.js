const statusMessages = {
    Signup_Success : 'Sign up successfully',
    Signup_Already_User_Exist : 'User already exist',
    Google_Verification_Failed : 'Google login verfication failed',
    Login_Success : 'Successfully logged-in',
    Login_Invalid_Email : 'Invalid Email Id provided',
    Login_Invalid_Password : 'Invalid password provided',

    User_Subscription_Feeds_Fetched_Success : "User subscription feeds fetched successfully",
    Site_Feeds_Fetched_Success : "Site feeds fetched successfully",

    Available_Rss_Sites_Fetched_Success : " Successfully fetched the available Rss sites",
    Rss_Detials_Fetched_Success : "Rss site fetched sucessfully",

    RssSite_Url_Already_Present : "Given Rss Url already present",
    RssSite_Added_Success : "Rss site added successfully",
    RssSite_Not_Registered: "Given Rss Site is not registered",

    RssSiteId_Not_Populated_Error : "Rss site id not populated",
    Subscribe_Success : "Rss feed Subscribed Sucessfully",
    Feed_Fetched_Success : "Rss site feeds fetched sucessfully",

    No_New_Feed_Published : "No new feed Published, try after some time",
    Feed_Fetched_And_Stored_Success : " Rss feeds fetched and stored successfully",
    Feed_Fetch_Done_Recently_Only : "Fetch recently only done, try after some time",
    Without_RssSiteDetails_Error : "Without rss Site details We cant fetch the feeds",
    Pub_Date_Not_Present_Error : "Feed parser result is not having pub date",
    FeedParser_Failed : "Feed parser result did not produce 200 Status code",
    Rss_Details_Error : "Proper Rss detail is not present",

    Already_Subscribed_Message : "Already Subscribed to this rss site",
    Fetch_Rss_Subscription_List_Success : "Sucessfully fetched the Subscription list",

}

module.exports = statusMessages;